...
%   C    : an m-by-r matrix, each column is a cluster centroid (endmember)
%   sol  : contains the tree structure

function [IDX, C, sol] = hierclust2nmf(M,r,algo,sol)
...
if nargin == 1 || isempty(r)
    ...
    if b == 'y' || b == 1
        if ~exist('L')
            H = input('What is the number of pixels in each row of your hyperspectral images? \n');
            L = n/H; 
        end
        r = Inf; 
...
while sol.count < r 
    ...
    for k = 1 : length(sol.leafnodes)
        ...
        if sol.e(sol.leafnodes(k)) == -1 && length(sol.K{sol.leafnodes(k)}) > 1
            ...
            [sol.U(:,sol.maxnode+1),sol.firstsv(sol.maxnode+1)] = reprvec(M(:,sol.K{sol.maxnode+1})); 
            [sol.U(:,sol.maxnode+2),sol.firstsv(sol.maxnode+2)] = reprvec(M(:,sol.K{sol.maxnode+2})); 
    ...
    elseif manualsplit == 1 % Split w.r.t. user visual feedback
        ...
        close all; affclust(sol.K(sol.leafnodes),H,L); 
    ...
    if b == -1
        ...
        [u1,s1] = reprvec(M(:,sol.K{sol.maxnode+1})); 
